...
public class ServerMain implements FileSystemObserver {
					...
					if(rememberedPeers!=null) {
						for(int i1=0; i1<rememberedPeers.length ;i1++){
							if(rememberedPeers[i1][0]!=null) {
								new UDP().Request(eventSync.get(i), rememberedPeers[i1]);
							}
						}
					}
				}
		...
		//UDP Server
			...
			try {
				while (true) {
					...
					listeningSocket.close();
					new UDP().response(packet,countUDP, fileSystemManager);
	...
	public static boolean Disconnection(String host, String port) {
		if(Configuration.getConfigurationValue("mode").equals("tcp")) {
			for(int i=0; i<max ;i++){
				if(th[i]!=null && th[i].Socket.getInetAddress().toString() == host 
				  && Integer.toString(th[i].Socket.getPort()) == port && th[i].isAlive()){
					th[i].interrupt();
					return true;
				}
			}
			if(thClient!=null) {
				for(int i=0; i<thClient.length ;i++){
					if(thClient[i]!=null && thClient[i].Socket.getInetAddress().toString() == host 
							  && Integer.toString(thClient[i].Socket.getPort()) == port && thClient[i].isAlive()){
			...
			if(rememberedPeers!=null) {
				for(int i=0; i<rememberedPeers.length ;i++){
					if(rememberedPeers[i][0]!=null && rememberedPeers[i][0] == host && rememberedPeers[i][1] == port) {
