								...
								if (Document.parse(request).get("command").equals("LIST_PEERS_REQUEST")) {
									...
									for (int i = 0; i < ServerMain.thClient.length; i++) {
										if (ServerMain.thClient[i] != null && ServerMain.thClient[i].isAlive()) {
											...
											peer.append("host", ServerMain.thClient[i].Socket.getInetAddress().getHostAddress());
									...
									for (int i = 0; i < ServerMain.th.length; i++) {
										if (ServerMain.th[i] != null && ServerMain.th[i].isAlive()) {
											Document peer = new Document();
											peer.append("host", ServerMain.th[i].Socket.getInetAddress().getHostAddress());
									...
									if(Configuration.getConfigurationValue("mode").equals("tcp")) {
										...
										for (int j = 0; j < Integer.parseInt(Configuration.getConfigurationValue("maximumIncomingConnections")) + 1; j++) {
											if (ServerMain.thClient[j] == null || !ServerMain.thClient[j].isAlive()) {
												ServerMain.thClient[j] = new Threads(Socket, j, "Client", sm.fileSystemManager);
												break;
											}
										}
