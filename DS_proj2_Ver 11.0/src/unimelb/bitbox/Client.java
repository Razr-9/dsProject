...
class Client {
		...
		try {
			...
			for (int i = 0; i < hostPorts.length; i++) {
				...
				for (int j = 0; j < Integer.parseInt(Configuration.getConfigurationValue("udpRetries")); j++) {
					...
					new UDPResponse().sendHandshake(HP.host, HP.port);
					DatagramSocket ClientSocket = new DatagramSocket(null);
					ClientSocket.setReuseAddress(true);
					ClientSocket.bind(
							new InetSocketAddress(Integer.parseInt(Configuration.getConfigurationValue("udpPort"))));
	...
	public static void main(String[] args) throws IOException, NumberFormatException, NoSuchAlgorithmException {
		...
		try (Socket socket = new Socket(cArg.getServer().host, cArg.getServer().port);) {
			...
			if (cArg.isValid()) {
				...
				Doc = new Document();
				String argsCommand = cArg.getCommand();
				if (argsCommand.equals("LIST_PEERS_REQUEST")) {
					Doc.append("command", "LIST_PEERS_REQUEST");
				} else if (argsCommand.equals("CONNECT_PEER_REQUEST")) {
					Doc.append("command", "CONNECT_PEER_REQUEST");
					Doc.append("host", cArg.getPeer().host);
					Doc.append("port", cArg.getPeer().port);
				} else if (argsCommand.equals("DISCONNECT_PEER_REQUEST")) {
				...
				if (msg != null) {
					...
					if (validResponse(response)) {
						System.out.println("Command resoponse Documentreceived.");
	...
	public static boolean validResponse(String msg) {
		...
		if (response.equals("LIST_PEERS_RESPONSE")) {
			...
			ArrayList<Document> peer = (ArrayList<Document>) Document.parse(msg).get("peers");
			if (!peer.isEmpty()) {
