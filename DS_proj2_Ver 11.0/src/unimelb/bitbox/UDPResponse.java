...
public class UDPResponse extends Thread{
	...
	public UDPResponse(DatagramPacket packet, int i, FileSystemManager fileSystemManager){
		this.packet = packet;
		this.i = i;
	...
	public void run() {
		...
		try {
			if(Document.parse(message).get("command").equals("HANDSHAKE_RESPONSE")) {
				System.out.println(Document.parse(message).toJson());
				String hostport = ((Document) Document.parse(message).get("hostPort")).toJson();
				
				ServerMain.rememberedPeers[ServerMain.countUDP][0] = Document.parse(hostport).get("host").toString();
				ServerMain.rememberedPeers[ServerMain.countUDP][1] = Long.toString((Long) Document.parse(hostport).get("port"));
				ServerMain.countUDP++;
			...
			if (fileModifyReady||fileCreateReady) {
				String pathName = Document.parse(message).get("pathName").toString();
				String fileDes = ((Document) Document.parse(message).get("fileDescriptor")).toJson();
				long fileSize = Document.parse(fileDes).getLong("fileSize");
	...
	public void sendHandshake(String host, int port) {
		...
		InetSocketAddress socketAddress = new InetSocketAddress(HP[0], Integer.parseInt(HP[1]));
		packetSent.setAddress(socketAddress.getAddress());
		packetSent.setPort(socketAddress.getPort());
