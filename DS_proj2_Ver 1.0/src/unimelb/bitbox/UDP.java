...
public class UDP {
	...
	public boolean response(DatagramPacket packet, int i, FileSystemManager fileSystemManager){
		...
		String message;
//		if(length<8192) {
//			message = new String(data, packet.getOffset(), length);
//		} else {
...
//		}
		...
		try {
			if (Document.parse(message).get("command").equals("HANDSHAKE_REQUEST")) {
				if(i<max) {
					...
					sendRequestDiagram(Doc, remoteHP);
	...
	private void sendDiagram(Document Doc, DatagramPacket packet) {
		byte [] dataSent = Doc.toJson().getBytes();
//		byte [] dataSentPadded;
		DatagramPacket packetSent = null;
//		int offSet = 0;
//		if(dataSent.length<8192) {
//			offSet = 8192-dataSent.length;
//			dataSentPadded = new byte [8192];
//		}
//		else {
//			dataSentPadded = dataSent;
//		}
		...
		DatagramSocket peer;
		try {
			peer = new DatagramSocket(Integer.parseInt(Configuration.getConfigurationValue("udpPort")));
	...
	public void Request(FileSystemEvent fileSystemEvent, String[] rememberedPeer) {
		...
		if (fileSystemEvent.event.toString().equals("FILE_CREATE")) {
			...
			sendRequestDiagram(Doc, rememberedPeer);
	...
	private void sendRequestDiagram(Document Doc, String[] rememberedPeer) {
		byte [] dataSent = Doc.toJson().getBytes();
//		byte [] dataSentPadded;
		DatagramPacket packetSent = null;
//		int offSet = 0;
