...
{
    public class HealText: DrawableGameComponent
    {
        public enum Units
        {
            Player,
        ...
        public HealText(Game game, int amount, Units target)
            : base(game)
        {
            position = target;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            game.spriteBatch.DrawString(font, number,
                                        new Vector2((position == Units.Player) ? (144) : (656 - font.MeasureString(number).X), y),
                                        Color.Lime * ((100 - t) / 100));
