...
class BALL {
	public:
		...
		void			   Cleanup(void);
		BALL(float x, float y, LPCSTR pth);
		float			   hspeed;
		float			   vspeed;
}*balls[MAX_BRICK_COUNT];
class BRICK {
	public:
		D3DXVECTOR3		   pos;
		LPDIRECT3DTEXTURE9 myTex;
...
BALL::BALL(float x, float y, LPCSTR pth) {
	pos.x = x;
	pos.y = y;
	pos.z = 0.0f;
		ZeroMemEx(myTex);
	...
	if(FAILED(D3DXCreateTextureFromFile(d3dDevice, pth, &myTex))) {
		MessageBox(0, "Texture creation in instance of BALL failed.", "Error", MB_ICONERROR | MB_OK);
	}
}
void BALL::DrawSprite(void) {
	pos.x += hspeed;
	pos.y += vspeed;
	mySpr ->Begin(D3DXSPRITE_ALPHABLEND);
		mySpr ->Draw(myTex, 0, 0, &pos, cWhite);
...
FLOAT LoadLevel(void) {
	...
	if(strcmp(__argv[0], "c:\\Users\\Elias\\Documents\\Visual Studio 2008\\Projects\\Pongout\\Debug\\Pongout.exe") == 0) {
		fp = fopen("c:\\Users\\Elias\\Documents\\Visual Studio 2008\\Projects\\Pongout\\Release\\data.djm", "r");
	}
	else if(strcmp(__argv[0], "c:\\Users\\helen\\Documents\\Visual Studio 2008\\Projects\\Pongout\\Debug\\Pongout.exe") == 0) {
		fp = fopen("c:\\Users\\helen\\Documents\\Visual Studio 2008\\Projects\\Pongout\\Release\\data.djm", "r");
	}
	else {
		fp = fopen("data.djm", "r");
	}
		for(i = 0; i < 16; i++) {
			bufchar[i] = (char)fgetc(fp); //magic number and version info
		}
		if(strcmp(bufchar, "PG-DJM;11/12/09;") != 1) {
		...
		buf = (float)fgetc(fp); //enemy difficulty; first byte is the integer value and second is floating value
		ENEMY_DIFFICULTY = (float)fgetc(fp) + ((float)fgetc(fp) / 100);
		buf2[0] = fgetc(fp);
		while(buf2[0] != 0xEF) {
			buf2[1] = fgetc(fp) * 16;
			buf2[2] = fgetc(fp) * 16;
			bricks[i] = new BRICK((float)buf2[1], (float)buf2[2], BRICKSPRS[buf2[0]]);
			if(bricks[i] == 0) {
				MessageBox(0, "Brick creation error.", "Error", MB_ICONERROR | MB_OK);
				return buf;
			}
			BrickCount++;
			buf2[0] = fgetc(fp);
